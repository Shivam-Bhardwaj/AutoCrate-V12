...
def test_hybrid_chooses_dimension_adjustment(base_inputs):
    """
    Tests the hybrid strategy choosing dimension adjustment when adjustment is small.
    """
    inputs = base_inputs.copy()
    inputs["strategy"] = "hybrid"
    inputs["front_panel_assembly_height"] = 97.0  # Requires a small adjustment
...
def test_legacy_hybrid_wrapper(base_inputs):
    ...
    """
    result = calculate_front_panel_components_hybrid(**base_inputs)
...
def test_legacy_dimension_adjustment_wrapper(base_inputs):
    ...
    """
    result = calculate_front_panel_components_with_dimension_adjustment(**base_inputs)
    assert result is not None
    assert 'strategy_used' in result
...
def test_main_execution(mock_print):
    """
