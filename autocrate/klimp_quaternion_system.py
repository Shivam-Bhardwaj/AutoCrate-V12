...
class KlimpQuaternionOrientation:
    ...
    def __post_init__(self):
        """Calculate direction vectors from quaternion if not provided."""
        if not self.direction_vectors:
            x_axis, y_axis, z_axis = self.quaternion.to_unit_vectors()
    ...
    @classmethod
    def identity(cls, klimp_id: int, pos_x: float, pos_y: float, pos_z: float) -> 'KlimpQuaternionOrientation':
        """Create identity orientation (no rotation)."""
        return cls(klimp_id, (pos_x, pos_y, pos_z), Quaternion.identity(), {})
    
    @classmethod
    def from_euler_angles(cls, klimp_id: int, pos_x: float, pos_y: float, pos_z: float,
        ...
        return cls(klimp_id, (pos_x, pos_y, pos_z), quat, {})
    
    @classmethod
    def from_axis_angle(cls, klimp_id: int, pos_x: float, pos_y: float, pos_z: float,
    ...
    def top_panel_orientation(cls, pos_x: float, pos_y: float, pos_z: float) -> 'KlimpQuaternionOrientation':
        ...
        return cls.identity(0, pos_x, pos_y, pos_z)
    ...
    def left_panel_orientation(cls, pos_x: float, pos_y: float, pos_z: float) -> 'KlimpQuaternionOrientation':
        ...
        return cls.from_euler_angles(0, pos_x, pos_y, pos_z, 0.0, 0.0, -90.0)
...
def interpolate_klimp_orientations(
    ...
    pos_x = start.position[0] + t * (end.position[0] - start.position[0])
    pos_y = start.position[1] + t * (end.position[1] - start.position[1])
    pos_z = start.position[2] + t * (end.position[2] - start.position[2])
    
    # Spherical linear interpolation for orientation
    interp_quat = start.quaternion.slerp(end.quaternion, t)
    
    return KlimpQuaternionOrientation(start.id, (pos_x, pos_y, pos_z), interp_quat, {})
