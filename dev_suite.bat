@echo off
REM AutoCrate V12 - ASCII-Compatible Development Suite
REM Fixed encoding issues - uses only ASCII characters
setlocal EnableDelayedExpansion

title AutoCrate V12 - ASCII Dev Suite
...
if /i "%choice%"=="8" goto KILL_SERVERS
...
if exist "node_modules\" (
    ...
    del temp_count.txt
    echo   [OK] Found !DIR_COUNT! installed packages
) else (
...
if errorlevel 1 (
    echo   [ERROR] Next.js not available
) else (
    echo   [OK] Next.js version:
    npx next --version
)
...
if exist "__pycache__" (
    ...
    if not exist "__pycache__" (
        ...
        echo   [ERROR] Failed to remove: __pycache__
    )
) else (
...
if exist "build" (
    ...
    echo   [OK] No build directory found
)
...
if exist "dist" (
    ...
    echo   [OK] No dist directory found
)
...
if exist "web\.next" (
    ...
    if not exist "web\.next" (
        echo   [OK] Removed: web\.next directory
        set /a CLEANED_COUNT+=1
    ) else (
        echo   [ERROR] Failed to remove: web\.next
    )
) else (
    echo   [OK] No web\.next directory found
)
...
if exist "web\out" (
    ...
    if not exist "web\out" (
        echo   [OK] Removed: web\out directory
        set /a CLEANED_COUNT+=1
    ) else (
        echo   [ERROR] Failed to remove: web\out
    )
) else (
    echo   [OK] No web\out directory found
)
...
if !TEMP_COUNT! equ 0 (
    ...
    echo   [OK] Removed: !TEMP_COUNT! temporary files
...
if exist "web\node_modules" (echo   [OK] web\node_modules) else (echo   [ERROR] web\node_modules missing)
echo.

echo Running Processes:
echo Checking for running servers...
netstat -an 2>nul | findstr ":5000" >nul && echo   [RUNNING] Port 5000 (API): OCCUPIED || echo   [FREE] Port 5000 (API): AVAILABLE
netstat -an 2>nul | findstr ":3000" >nul && echo   [RUNNING] Port 3000 (Web): OCCUPIED || echo   [FREE] Port 3000 (Web): AVAILABLE
echo.
...
if errorlevel 1 (
    ...
    echo   [ACTION] Terminating Node.js processes...
    taskkill /f /im node.exe 2>nul
    if errorlevel 1 (
        echo   [ERROR] Failed to kill some Node.js processes
    ) else (
...
if errorlevel 1 (
    ...
    echo   [ACTION] Terminating npm processes...
    taskkill /f /im npm.exe 2>nul
    echo   [OK] npm processes terminated
)

echo.
echo [3/4] Killing Python API servers...
set PYTHON_KILLED=0
for /f "tokens=5" %%a in ('netstat -ano 2^>nul ^| findstr ":5000"') do (
    echo   [ACTION] Killing process %%a on port 5000...
    taskkill /f /pid %%a 2>nul
    set PYTHON_KILLED=1
)
for /f "tokens=5" %%a in ('netstat -ano 2^>nul ^| findstr ":8000"') do (
    echo   [ACTION] Killing process %%a on port 8000...
    taskkill /f /pid %%a 2>nul
    set PYTHON_KILLED=1
)
if !PYTHON_KILLED! equ 0 (
    echo   [INFO] No Python API servers found on ports 5000/8000
) else (
    echo   [OK] Python API servers terminated
)

echo.
echo [4/4] Verifying ports are free...
timeout /t 2 >nul
netstat -an 2>nul | findstr ":3000" >nul && echo   [WARNING] Port 3000: Still occupied || echo   [OK] Port 3000: Free
netstat -an 2>nul | findstr ":5000" >nul && echo   [WARNING] Port 5000: Still occupied || echo   [OK] Port 5000: Free
netstat -an 2>nul | findstr ":8000" >nul && echo   [WARNING] Port 8000: Still occupied || echo   [OK] Port 8000: Free
...
echo Ports 3000, 5000, and 8000 should now be available.
echo You can restart servers using options 1-3.
echo.
pause
goto MAIN_MENU
